...
public class Target {
    ...
    private final Pose2d BLUE_GOAL = new Pose2d(4.03, 4.035, new Rotation2d());
        ...
        public void updateTargetStatus(int aprilTagsID){
            ...
            if (alliance.isPresent()){
                if (alliance.get() == Alliance.Red && ((aprilTagsID >= 2 && aprilTagsID <= 5) || (aprilTagsID >= 8 && aprilTagsID <= 11))){
                    isValidTarget = true;
                    TargetPose = RED_GOAL;
                }
                else if (alliance.get() == Alliance.Blue && ((aprilTagsID >= 18 && aprilTagsID <= 21) || (aprilTagsID >= 24 && aprilTagsID <= 27))){
                    isValidTarget = true;
    ...
    public double detDistanceToTarget(Pose2d robotPose){
